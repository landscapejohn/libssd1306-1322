...
int ssd1322_run_cmd(ssd1322_6800_t *oled, ssd1322_cmd_t cmd, uint8_t *data, size_t dlen)
{
...
void ssd1322_6800_close(ssd1322_6800_t *oled)
{
    if (oled) {
...
int ssd1306_i2c_display_update(ssd1322_6800_t *oled, const ssd1322_framebuffer_t *fbp)
{
    FILE *err_fp = SSD1322_GET_ERRFP(oled);
    ...
    fprintf(err_fp, "INFO: Wrote %zd bytes of screen buffer to device\n", nb);
    return 0;
}

int ssd1306_i2c_display_clear(ssd1322_6800_t *oled)
{
...
int ssd1306_i2c_display_initialize(ssd1322_6800_t *oled)
{
    int rc = 0;
    uint8_t data[2];
    FILE *err_fp = SSD1322_GET_ERRFP(oled);
...
ssd1322_6800_t *ssd1306_i2c_open(uint16_t width, uint16_t height, FILE *logerr)
{
    ssd1322_6800_t *oled = NULL;
    int rc = 0;
